...
export async function bindFolder(folderAbsPath, dbAbsPath) {
  ...
  // Where the workspace DB lives. Default to a `Throughline` subfolder so the
  // shared root stays tidy (the dyad partner doesn't see state.json at top level)
  // and multi-user metadata can live alongside it later. The DB must sit inside
  // the chosen folder.
  const db = (typeof dbAbsPath === 'string' && dbAbsPath)
    ? dbAbsPath
    : join(folderAbsPath, 'Throughline', 'state.json');
  if (!isAbsolute(db)) throw new Error('data file path must be an absolute path');
  const relRootDb = relative(folderAbsPath, db);
  if (relRootDb.startsWith('..') || isAbsolute(relRootDb)) {
    throw new Error('data file must be inside the chosen folder');
  }
  // Create the parent (e.g. the Throughline subfolder) so a brand-new workspace
  // can be written. The file itself is created lazily on first save.
  ...
  if (dbStat && dbStat.isFile() && dbStat.size === 0) {
    warning =
      'The workspace file looks empty — if it is an online-only OneDrive file, ' +
      'right-click the folder in File Explorer → "Always keep on this device".';
  }
  return { onedriveRoot: folderAbsPath, dbPath: db, warning };
}

// Count containers by type in a state.json (best-effort; null if unreadable).
async function summarizeDb(abs) {
  try {
    const doc = JSON.parse(await readFile(abs, 'utf8'));
    const cs = Array.isArray(doc.containers) ? doc.containers : [];
    const by = (t) => cs.filter((c) => c && c.type === t).length;
    return { projects: by('project'), programs: by('program'), references: by('reference_file') };
  } catch {
    return null;
  }
}

// For a chosen lens root, report the candidate workspace-DB locations so the
// wizard can default to the tidy `Throughline` subfolder and surface an existing
// workspace (with its project/program counts) to reuse. Recommended first.
export async function dbInfo(rootAbs) {
  if (typeof rootAbs !== 'string' || !isAbsolute(rootAbs)) {
    throw new Error('folder must be an absolute path');
  }
  const home = setupRoot();
  const relToHome = relative(home, rootAbs);
  if (relToHome.startsWith('..') || isAbsolute(relToHome)) {
    throw new Error('folder must be inside your user profile');
  }
  const candPaths = [
    { rel: 'Throughline/state.json', abs: join(rootAbs, 'Throughline', 'state.json'), recommended: true },
    { rel: 'state.json', abs: join(rootAbs, 'state.json'), recommended: false },
  ];
  const candidates = [];
  for (const c of candPaths) {
    const cst = await stat(c.abs).catch(() => null);
    const exists = !!(cst && cst.isFile());
    candidates.push({
      path: c.abs,
      rel: c.rel,
      recommended: c.recommended,
      exists,
      size: exists ? cst.size : 0,
      summary: exists ? await summarizeDb(c.abs) : null,
    });
  }
  return { root: rootAbs, default: candPaths[0].abs, candidates };
}
