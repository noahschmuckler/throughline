...
export async function listFolder(rel = '', rootAbs = rootDir()) {
  const { path, folders, files } = await listUnder(rootAbs, rel, 'ONEDRIVE_ROOT');
...
export async function openFile(rel, rootAbs = rootDir()) {
  const abs = resolveWithin(rootAbs, rel, 'ONEDRIVE_ROOT');   // M3: confine to the given circle's root
  ...
  if (process.env.THROUGHLINE_OPEN_DRYRUN !== '1') {
    try {
      ...
      return { ok: false, path: toRelUnder(rootAbs, abs), command, args, error: e.message };
    }
  }
  return { ok: true, path: toRelUnder(rootAbs, abs), command, args };
}
