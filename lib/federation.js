...
import { readStateAt, writeStateAt } from './store.js';
...
export async function loadFederated() {
  ...
  const pairs = [];
  for (const c of registry) pairs.push({ circle: c, state: await readStateAt(c.statePath) });
...
export async function saveFederated(incoming) {
  const registry = await discoverCircles();
  const primary = registry.find(c => c.primary) || registry[0];
  ...
  for (const c of registry) {
    ...
    const theirs = await readStateAt(c.statePath);          // current disk
    ...
    if (contentKey(merged) !== contentKey(theirs)) await writeStateAt(merged, c.statePath);
