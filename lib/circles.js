...
// Federation (the sibling-circle scan) is ON BY DEFAULT (2026-06-17). It scans
// circlesRoot() — THROUGHLINE_CIRCLES_ROOT if set, else the parent of
// ONEDRIVE_ROOT (the OneDrive top level holding the user's shared folders) — for
// immediate subfolders that ARE circles (`<folder>/Throughline/state.json`).
// A box with no sibling circles just gets a one-entry registry (the primary) and
// behaves exactly as a single-folder install — so defaulting this on is safe.
// Set THROUGHLINE_CIRCLES=off (or 0/false/no) to force single-circle-only.
export function federationEnabled() {
  const v = String(process.env.THROUGHLINE_CIRCLES ?? '').trim().toLowerCase();
...
async function ensureWorkspaceIdentity(statePath, folderAbs, { mintId = randomUUID } = {}) {
  // Classify the file: missing / blank (safe to initialize) vs valid JSON vs
  // malformed-nonempty (leave it alone — never crash discovery, never clobber).
  let doc = null;
  let initialize = false;   // file is missing or blank → safe to create a fresh one
  try {
    const txt = await readFile(statePath, 'utf8');
    if (txt.trim()) doc = JSON.parse(txt);    // may throw → malformed
    else initialize = true;                    // 0-byte / whitespace
  } catch (err) {
    if (err.code === 'ENOENT') initialize = true;
    else console.warn(`[circles] ${statePath} unparseable: ${err.message} (left untouched)`);
  }
  ...
  const persist = async (out) => {
    ...
    try { await rename(tmp, statePath); } catch { await writeFile(statePath, body, 'utf8'); }
  };

  if (initialize) {
    // A blank/new circle file: write a valid empty doc NOW so its id is STABLE
    // from the first load (otherwise a fresh uuid would be minted every GET and
    // write-routing for that circle would never match).
    await persist({ schema_version: 3, workspace: ws, containers: [], entries: [], atoms: [], people_meta: {} });
  } else if (changed && doc) {
    // Existing valid file missing an id (pre-circles migration) — stamp it,
