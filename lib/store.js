...
// Read a state document from an explicit path (M3: one file per circle).
// Missing OR blank/whitespace-only (a freshly-touched circle file) → empty state
// — a brand-new circle must not crash the federated load (T38). A non-empty but
// malformed file still throws (the federation isolates it per-circle so one bad
// file can't blank the others, and refuses to overwrite it).
